...
function handleCompile() {
  ...
  hexOutput.textContent =
    result.hex || "// La salida hexadecimal aparecerá aquí después de compilar";
...
function updateErrors() {
  ...
  if (currentErrors.length === 0) {
    ...
    errorsList.innerHTML = currentErrors
      .map(
        (error) => `
...
function switchTab(tabName) {
  ...
  // Mostrar contenido
  document
    .getElementById("hexTab")
    .classList.toggle("active", tabName === "hex");
  document
    .getElementById("errorsTab")
    .classList.toggle("active", tabName === "errors");
}
...
function switchInstructionTab(tabName) {
  ...
  // Mostrar contenido
  document
    .getElementById("allInstructionsTab")
    .classList.toggle("active", tabName === "all");
  document
    .getElementById("quickRefTab")
    .classList.toggle("active", tabName === "quick");
}
...
function renderInstructions(filter = "") {
  ...
  if (filter) {
    ...
    if (filtered.length === 0) {
      container.innerHTML =
        '<p style="color: #64748b; text-align: center; padding: 2rem;">No se encontraron instrucciones</p>';
    }
  } else {
    // Mostrar por categorías
    Object.entries(INSTRUCTION_CATEGORIES).forEach(
...
function createInstructionCard(name, info) {
  ...
  card.innerHTML = `
        <div class="instruction-header">
            ...
            <span class="instruction-opcode">0x${info.opcode
              .toString(16)
              .toUpperCase()
              .padStart(4, "0")}</span>
        </div>
